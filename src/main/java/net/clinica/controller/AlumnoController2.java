...
public class AlumnoController2 {
	@Autowired
	private AlumnoService2 servicioAlu;
	
	//select *from tb_medicamento --->JSON
	@GetMapping("/lista")
	public ResponseEntity<List<Alumno2>> lista() throws Exception{
	...
	public ResponseEntity<Alumno2> buscar(@PathVariable("codigo") Integer cod) throws Exception{
		Alumno2 med=servicioAlu.buscarPorCodigo(cod);
		//validar
		if(med==null)
	...
	public ResponseEntity<Alumno2> registrar(@RequestBody Alumno2 med) throws Exception{
		...
		Alumno2 bean=servicioAlu.registrar(med);
	...
	public ResponseEntity<Alumno2> actualizar(@RequestBody Alumno2 med) throws Exception{
		Alumno2 bean=servicioAlu.buscarPorCodigo(med.getCodigo());
		//validar
		if(bean==null)
			throw new NotFoundException();
		else {
	...
	public ResponseEntity<Void> eliminar(@PathVariable("codigo") Integer cod) throws Exception{
		Alumno2 bean=servicioAlu.buscarPorCodigo(cod);
		//validar
		if(bean==null)
			throw new NotFoundException();
		else
